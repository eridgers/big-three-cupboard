...
function itemCreate(name, brand, weight, cost, quantity, description, category, image, cb) {
    itemdetail = {name:name, brand:brand, weight:weight, cost:cost, 
                quantity:quantity, description:description, category: category, image: image}
...
function categoryCreate(name, description, image, cb) {
  var category = new Category({ name: name, description: description, image: image });
...
function brandCreate(name, description, logo, cb){
    var brand = new Brand({name: name, description: description, logo: logo});
...
function createItems(cb) {
    async.series([
        ...
        // tents
        function(callback) {
            itemCreate('Fly Creek UL1', brands[0], 750, 325, 8, 'A great lighweight option, semi-freestanding', categories[0], '/images/fly-creek-ul-1.jpg', callback);
        },
        function(callback) {
            itemCreate('Duplex', brands[4], 550, 600, 2, 'Specifically designed for ultralight backpacking and thru-hiking, the award-winning Duplex hits the sweet spot when it comes to size, weight, and features. This tent will allow you to push your limits and explore over greater distances, whether you’re finding the best route on the Pacific Crest Trail, or losing yourself in the beauty of the Swiss Alps. This tent is roomy, well-ventilated, packs down small, offers simple set up, and can be set up almost anywhere! You won’t find a lighter functional version of this tent anywhere on the market.', categories[0], '/images/duplex.jpg', callback);
        },
        function(callback) {
...
function createCategories(cb) {
    async.series([
        function(callback) {
            categoryCreate("Tents", "The perfect place to lay your head at night", "/images/tent.png",callback);
        },
        function(callback) {
            categoryCreate("Backpacks", "Jam all your stuff in and sling it on your back!", "/images/backpack.jpg ", callback);
        },
        function(callback) {
            categoryCreate("Sleeping Bag", "Get snuggly, stay warm out there", "/images/sleeping-bag.png", callback);
        },
        function(callback) {
            categoryCreate("Sleeping Pad", "I guess its the big four, these keep you warm at night too", "/images/sleeping-pad.jpg", callback);
        }
        ],
        // optional callback
        cb);
...
function createBrands(cb) {
    async.series([
         // brands
        function(callback){
            brandCreate('Big Agnes', 'Big Agnes inspires you to get outside with our comfortable and durable down sleeping bags, lightweight tents, self-inflating sleeping pads, and down jackets.', '/images/big-agnes.png', callback);
        },
        function(callback){
            brandCreate('Mountain Hardwear', 'Mountain Hardwear brings elevated performance to mens & womens clothing, jackets, tents, sleeping bags, backpacks, equipment, gear and accessories.', '/images/mountain-hardwear.png', callback);
        },
        function(callback){
            brandCreate('Thermarest', 'Offering a better night under the stars, we have been responsibly crafting the best camp gear since 1972.', '/images/thermarest.png', callback);
        },
        function(callback){
            brandCreate('Osprey', 'At the heart of Osprey is a passion for the outdoors. Innovative, high performance gear that reflects a love of adventure and devotion to the outdoors. High quality packs for any adventure and season.', '/images/osprey.jpg', callback);
        },
        function(callback){
            brandCreate('Zpacks', 'Zpacks Offers Lightweight Backpacking Gear: Ultralight Dyneema Composite Fabric / Cuben Fiber Tents Tarps Backpacks Down Sleeping Bags Rain Gear.', '/images/zpacks.jpg', callback);
        },
        function(callback){
            brandCreate('Gossamer Gear', 'Functional ultralight backpacking and hiking gear designed by hikers. Providing you the best in lightweight outdoor and minimalist gear.', '/images/gossamer-gear.jpg', callback);
        }
        ],
        // Optional callback
        cb);
